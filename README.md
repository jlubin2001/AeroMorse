...
| `KEYCODE_REFERENCE.md` / `keycode_reference.htm` | Reference of every valid `Keycode.NAME` for `morse_map.py` (letters, number keys, navigation, punctuation, F-keys, keypad, modifiers), plus how to produce shifted symbols (`:` `{` `}` …). The `.htm` is a printable sheet styled like the cheat sheet; the `.md` opens in the Markdown viewer. Extracted from the `adafruit_hid` 9.x library on the devices. |
